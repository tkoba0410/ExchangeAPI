...
Factory は Transport/Adapter を組み合わせて、bitFlyer 向けのクライアントを簡単に構築するヘルパを提供するレイヤーです。DI を使わないシナリオでも最小コードでクライアントを得られるのが役割です。

## 主なコンポーネント
- `BitflyerClientFactory`（adapter 側）: API キー/シークレットを渡して `BitflyerExchangeClient`（Facade）を構築。HTTP/署名/Raw API/Adapters/Apis/Facade をまとめて組み立てる。
