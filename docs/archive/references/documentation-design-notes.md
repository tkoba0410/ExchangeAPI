...
- 本文書内に現れる「正本」「必ず」「MUST/NG」などの語は、当時の検討メモを保存した記述であり、現行の規範を定義しない。
- 現行の判断は `docs/index.md` から参照される Normative 文書のみを用いる。
...
* 旧 TopSpec / boundaries / guide を `docs/archive/references/` 配下へ移動する
