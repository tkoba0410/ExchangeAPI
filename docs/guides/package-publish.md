...
- 本書の current command 例は `v3.4.0` の publish 手順を示す
- `v3.4.0` publish 前の確認では、`3.4.0-local.*` のような local package version を使う
- `v3.4.0` publish 前の最終確認では、publish/tag/release は実行せず、`3.4.0-local.final` などの local version で preflight する
...
```bash
bash scripts/run-release-preflight.sh 3.4.0-local.preflight
```

safe live verification まで含める場合だけ、次のように明示 opt-in する。

```bash
EXCHANGEAPI_RUN_SAFE_LIVE_PREFLIGHT=1 bash scripts/run-release-preflight.sh 3.4.0-local.preflight
```
...
```bash
dotnet nuget push "local/nuget/ExchangeApi.Primitives.3.4.0.nupkg" \
...
- GitHub Packages publish: `ExchangeApi.Exchanges.Bitflyer 3.4.0`
- GitHub Packages publish: `ExchangeApi.Exchanges.Binance 3.4.0`
- GitHub Packages publish: `ExchangeApi.Primitives 3.4.0`
- GitHub Packages publish: `ExchangeApi.Optional.Credentials 3.4.0`
...
- `stage` 系、`v1.0.0`、`v2.0.0`、`v2.1.0`、`v2.2.0`、`v3.0.0` は履歴であり、package の current working baseline は `v3.4.0`
