...
この smoke は一時 consumer project を作成し、`ExchangeApi.Exchanges.Bitflyer`、`ExchangeApi.Exchanges.Binance`、`ExchangeApi.Optional.Credentials`、`ExchangeApi.Optional.Logging`、`ExchangeApi.Optional.Reactive`、`ExchangeApi.Optional.Testing` を local feed から restore して build / run する。
実 API には接続しない。
`BitflyerClientFactory`、`BinanceClientFactory`、`BitflyerRealtimeClientFactory`、`BitflyerRealtimeChannels`、`BitflyerRealtimeStreamEvent<T>`、`BitflyerRealtimeClientOptions`、`PlainTextApiCredentialProviderFactory`、`Redactor`、`ToObservable(...)`、`RealtimeReplayFrame`、`BitflyerRealtimeReplayRunner` を参照できること、secret value が `[REDACTED]` になること、smoke output が secret-free であることを確認する。
