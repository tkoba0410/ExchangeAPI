...
詳細は `docs/reference/utilities.md` を参照する。
...
※ 取引所実装間で「統一する/しない」を運用として管理する場合は、参考として `docs/archive/references/exchange-parity-policy.md` を用いる。
...
* 取引所機能網羅を優先する利用（Bot / 高度利用）では、
  取引所別 **Normalized 層**を利用してよい（MAY）。
* ただし Normalized は公開安定契約ではないため、
  互換保証外（追従前提）として扱う（MUST）。
* Contracts は「取引所横断で共通に成立する最小機能」に限定する（MUST）。
* Raw / Wire は内部実装層であり、外部利用境界として扱ってはならない（MUST NOT）。
