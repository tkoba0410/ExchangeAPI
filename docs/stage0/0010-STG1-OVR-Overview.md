...
### 5.7 HTTP クライアントおよびトランスポート方針（Stage 1）

Stage 1 における bitFlyer Adapter の HTTP クライアントおよびトランスポート層の方針を以下に定める。

#### 5.7.1 HttpClient のライフタイム

* `IBitflyerPublicApi` の実装は、HTTP 通信に .NET 標準の `HttpClient` を利用する。
* `HttpClient` は原則として **アプリケーション側（DI コンテナなど）から注入されるものを再利用**し、Adapter 自身が `new HttpClient()` してライフタイムを管理しない。
* 1 つの `HttpClient` インスタンスを複数の `IBitflyerPublicApi` / `BitflyerExchangeClient` から共有してよい。
* `IBitflyerPublicApi` / `BitflyerExchangeClient` 自体は `IDisposable` を実装せず、`HttpClient` の破棄責務はアプリケーション側に委ねる。

#### 5.7.2 Base URL / エンドポイント

* Stage 1 では bitFlyer Public REST API の Base URL を `https://api.bitflyer.com` とする。
* `/v1/getticker`（参考：8.1 節）を使用する。
* 将来の拡張として、`BitflyerOptions` などで Base URL を差し替えられる設計を許容する。

#### 5.7.3 JSON シリアライゼーション

* JSON パースには `System.Text.Json` を使用する。
* 以下を推奨設定とする：

  * `JsonSerializerOptions.PropertyNamingPolicy = null`
  * 取引所側に未知フィールドが追加されても **エラーにせず無視** する。
* JSON のパースに失敗した場合は、`ExchangeApiException` にラップし `RemoteError` に可能な範囲で情報を格納する。

#### 5.7.4 タイムアウトおよびリトライ

* `HttpClient.Timeout` の既定値（100 秒）は利用せず、**5〜10 秒程度**を推奨とする。
* Stage 1 では内部自動リトライを**行わない**。

  * リトライ戦略は Stage 2 以降の拡張対象とする。
  * 必要に応じて利用者側が Polly などで管理する想定とする。

#### 5.7.5 Content-Type / エンコーディング

* 期待される Content-Type は `application/json; charset=UTF-8` とする。
* Content-Type が完全一致しなくても JSON としてパースできる場合は処理を継続する。
* 文字コードは UTF-8 前提とする。
