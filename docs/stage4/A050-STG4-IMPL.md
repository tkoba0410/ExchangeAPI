...
## 1. 抽象/ドメイン
- `IMarketDataApi` / `ITradingApi` / `IAccountApi` / `IMarginAccountApi` / `IRealtimeMarketDataApi` / `IExchangeInfoApi` のメソッドセットを凍結する。MarketData には Candlesticks（GetCandlesticks）を含め、bittrade を初期実装ターゲットとする（bitFlyer など非対応は NotImplemented を返す）。
...
- フォルダ/プロジェクト構成の指針（提案）
  - `core/`: 抽象IF・共通DTO・共通エラー（取引所非依存）
  - `transport/`: HTTP/WS クライアント、RestClient/Signer、シリアライズ等の I/O 基盤（取引所非依存）
  - `adapter.bitflyer/`: bitFlyer 固有の REST/WS 実装・DTO/マッピング
  - `factory/`: DI 組み立て（機能ごとの登録オン/オフを扱う）
