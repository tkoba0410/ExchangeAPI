...
- 実装済み: TestFactory + モック Transport で正式フロー（残高→注文→約定確認→履歴→決済※反対売買→キャンセル→ポジション→証拠金）を劣化環境下で通す E2E テストを追加済み。
- 追加方針: 本番近似計測を行い、Timeout/Retry/RL/CB のデフォルト値を確定する（計測結果を SPEC/コードに反映）。
