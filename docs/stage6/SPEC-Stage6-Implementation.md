...
- 実装済み: TestFactory + モック Transport で代表フロー簡略版（残高→注文→約定確認→履歴）を劣化環境下で通す E2E テストを追加済み。
- 追加方針: 本番近似計測を行い、Timeout/Retry/RL/CB のデフォルト値を確定。正式フロー（決済/履歴詳細まで）を劣化環境で通す E2E を追加して DoD を「正式フロー完了」に更新する。
- 進捗補足: 劣化環境E2E を履歴/キャンセル/ポジション/証拠金まで拡張済み（決済フェーズは今後の検討）。
