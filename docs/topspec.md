...
* 本文書は、本リポジトリにおける**技術仕様・設計規範**の正本（Normative）である。
* 公開 API の契約に関しては Contracts 文書を、設計判断の裁定に関しては Governance 文書をそれぞれ正本とする。
* 技術仕様・設計規範については、本書以外の文書に記載された内容がいかなる場合も本書に優先することはない。
