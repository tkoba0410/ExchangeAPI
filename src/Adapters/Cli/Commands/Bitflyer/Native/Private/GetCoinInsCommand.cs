...
public static class GetCoinInsCommand
{
    public static CommandDescriptor Create()
    {
        return new CommandDescriptor
        {
            ...
            UsageExamples =
            [
                "exchangeapi bitflyer native private get-coin-ins --count 10",
                """exchangeapi bitflyer native private get-coin-ins --request-json '{"count":10}'""",
                "exchangeapi bitflyer native private get-coin-ins --request-template",
            ],
            IsWrite = false,
            BindRequestAsync = BindRequestAsync,
            DescribeRequest = static request =>
            {
                var typed = (GetCoinInsRequest)request;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return JsonInputReader.Deserialize<GetCoinInsRequest>(jsonInput.Content!);
        }
        ...
        return RequestBindingResult.Success(new GetCoinInsRequest
        {
            Count = count,
            Before = before,
            After = after,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var call = await bundle.Private.GetCoinInsAsync((GetCoinInsRequest)request, cancellationToken);
        return ExecutionOutcome.FromCall(new CommandPath("bitflyer", "native", "private", "get-coin-ins"), call);
    }
}
