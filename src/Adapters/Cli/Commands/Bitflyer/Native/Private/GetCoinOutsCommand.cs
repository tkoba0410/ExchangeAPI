...
public static class GetCoinOutsCommand
{
    public static CommandDescriptor Create()
    {
        return new CommandDescriptor
        {
            Path = new CommandPath("bitflyer", "native", "private", "get-coin-outs"),
            EndpointId = "GetCoinOuts",
            Summary = "bitFlyer native private coin outs",
            ...
            CanonicalJsonExample = """exchangeapi bitflyer native private get-coin-outs --request-json '{"count":10}'""",
            CommandOptions =
            [
            ...
            UsageExamples =
            [
                "exchangeapi bitflyer native private get-coin-outs --count 10",
                """exchangeapi bitflyer native private get-coin-outs --request-json '{"count":10}'""",
                "exchangeapi bitflyer native private get-coin-outs --request-template",
            ],
            IsWrite = false,
            BindRequestAsync = BindRequestAsync,
            DescribeRequest = static request =>
            {
                var typed = (GetCoinOutsRequest)request;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return JsonInputReader.Deserialize<GetCoinOutsRequest>(jsonInput.Content!);
        }
        ...
        return RequestBindingResult.Success(new GetCoinOutsRequest
        {
            Count = count,
            Before = before,
            After = after,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var call = await bundle.Private.GetCoinOutsCallAsync((GetCoinOutsRequest)request, cancellationToken);
        return ExecutionOutcome.FromCall(new CommandPath("bitflyer", "native", "private", "get-coin-outs"), call);
    }
}
