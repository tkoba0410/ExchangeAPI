...
public static class OrderBookLevelExtensions
{
    ...
    /// <summary>
    /// 買い（asks 側）で指定価格以下までに約定可能な合計サイズを返す。
    /// asks は昇順を想定（昇順でなくても条件で抽出する）。
    /// </summary>
    public static decimal CalcExecutableSizeForBuy(this IEnumerable<OrderBookLevel> asks, decimal maxPrice)
    {
    ...
    /// <summary>
    /// 売り（bids 側）で指定価格以上までに約定可能な合計サイズを返す。
    /// bids は降順を想定（降順でなくても条件で抽出する）。
    /// </summary>
    public static decimal CalcExecutableSizeForSell(this IEnumerable<OrderBookLevel> bids, decimal minPrice)
    {
