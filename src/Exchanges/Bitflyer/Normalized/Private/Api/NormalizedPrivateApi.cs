...
internal sealed class NormalizedPrivateApi
{
    ...
    public async Task<Call<PrivateRequests.GetChildOrdersByOrderKeyRequest, OrderStatus>> GetChildOrdersCallAsync(
        Symbol symbol,
        OrderKey orderKey,
        CancellationToken cancellationToken = default)
    {
        ...
        if (symbol.IsEmpty)
        {
            return CreateImmediateError<PrivateRequests.GetChildOrdersByOrderKeyRequest, OrderStatus>(
                callRequest,
                "Bitflyer.GetOrder",
                new CallError(CallErrorKind.Semantic, "Symbol is required."));
        }
        ...
        if (!TryGetProductCode(marketCall, out var productCode, out var marketError))
        {
            return CreateCallError<PrivateRequests.GetChildOrdersByOrderKeyRequest, OrderStatus>(
                marketCall,
                callRequest,
                "Bitflyer.GetOrder",
                marketError!);
        ...
        {
            return CreateNotSupported<PrivateRequests.GetChildOrdersByOrderKeyRequest, OrderStatus>(
                callRequest,
                component: "Bitflyer.Trading",
                feature: "GetOrder",
                reason: $"orderKey.Kind={orderKey.Kind}",
        ...
        return CreateCall(
            ...
            orders =>
            {
                ...
                if (order is null)
                {
                    return MapResult<OrderStatus>.Fail(
                        new CallError(CallErrorKind.Mapping, $"Order not found. symbol={symbol} orderKey={orderKey}"));
                }

                var status = CommonMapper.MapOrderStatus(order.ChildOrderStatusState);
                ...
                return MapResult<OrderStatus>.Ok(new OrderStatus(
                    ProductCode: productCode,
                    Key: resolvedKey,
                    Status: status,
