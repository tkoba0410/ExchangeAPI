...
`stage10.md` が移行途中の判断や旧構成との折衷を含むのに対し、本書は `Facade + Endpoint Module` を前提に、今後の実装方針をまっさらな設計として定義する。
...
- これは shared transport 語彙の再利用であり、Stage10B の設計正本が `Wire` であることを意味しない
...
- `Mapping`
  ...
  - Stage10B の初期 `Native` endpoint では原則として使わない
