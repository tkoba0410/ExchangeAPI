...
public sealed class BitflyerWireTransportTests
{
    [Fact]
    public async Task SendAsync_WithTickerAliasEnabled_SendsAliasPathButKeepsCanonicalRequest()
    {
        ...
        var transport = new BitflyerWireTransport(restClient, useTickerAliasPath: true);
    ...
    {
        var restClient = new CaptureRestClient();
        var transport = new BitflyerWireTransport(restClient, useTickerAliasPath: false);
        var request = new WireCallSpec(
            Method: "GET",
            Path: "/v1/getticker",
            EndpointId: "GetTicker");
